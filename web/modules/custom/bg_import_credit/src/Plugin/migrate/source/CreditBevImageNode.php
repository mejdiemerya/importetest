 ...
 *
 * @MigrateSource(
 *   id = "credit_bev_image_node"
 * )
 */
final class CreditBevImageNode extends SqlBase {
  ...
  public function query(): SelectInterface {
    ...
    $query ->condition("n.type","credit_bev_image" );
  ...
  public function getFieldRecordTid($fieldName, $tid){
    $result = $this->select("field_data_{$fieldName}", 'f')
      ...
      ->condition('f.bundle', 'credit_bev_image', '=')
...
foreach ($contentFile as $row) {
  $file = File::create([
    ...
    'uri' => 'public://credits/bev/' . basename($row['uri']),
    'status' => 1,
    'uid' => $uid,
  ]);
  $file->save();
