...
trait CommerceMigrateCoreTestTrait {
  ...
  protected function assertFileEntity($id, $name, $uri, $mime, $size, $uid) {
    ...
    $this->assertSame($size, (string) $file->getSize());
  ...
  protected function assertUserEntity($id, $label, $mail, $password, $login, $blocked, $langcode, $timezone, $init, array $roles) {
    ...
    if ($langcode == '') {
      $this->assertSame('en', $user->language()->getId());
      $this->assertSame($default_langcode, $user->getPreferredLangcode());
      $this->assertSame($default_langcode, $user->getPreferredAdminLangcode());
    }
    elseif ($language_manager->getLanguage($langcode) === NULL) {
      $this->assertSame($default_langcode, $user->language()->getId());
      $this->assertSame($default_langcode, $user->getPreferredLangcode());
      $this->assertSame($default_langcode, $user->getPreferredAdminLangcode());
    }
    else {
      $this->assertSame($langcode, $user->language()->getId());
      $this->assertSame($langcode, $user->getPreferredLangcode());
      $this->assertSame($langcode, $user->getPreferredAdminLangcode());
    }
