...
class CommercePrice extends ProcessPluginBase {
  ...
  public function transform($value, MigrateExecutableInterface $migrate_executable, Row $row, $destination_property) {
    ...
    if (isset($value['amount']) && isset($value['currency_code'])) {
      if (!isset($value['fraction_digits'])) {
        $currencyRepository = new CurrencyRepository();
        $value['fraction_digits'] = $currencyRepository->get($value['currency_code'])->getFractionDigits();
      }
      if (isset($value['fraction_digits']) && $value['fraction_digits'] >= 0) {
        return [
